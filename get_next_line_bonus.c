...
/*   Updated: 2025/12/07 16:20:19 by brouane          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	working_line[fd] = read_line(working_line[fd], fd);
	if (!working_line[fd])
		return (NULL);
	line = next_line(working_line[fd]);
	working_line[fd] = new_line(working_line[fd]);
	if (working_line[fd] && !*working_line[fd])
	{
		free(working_line[fd]);
		working_line[fd] = NULL;
	}
	return (line);
}
